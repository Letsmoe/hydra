...
(() => {
	...
	if (config.watch !== false && (typeof config.watch === "string")) {
		...
		const openWatcher = (dir: string, callback: (type: string, path: string) => void) => {
		...
		const configWatcher = openWatcher(PROJECT_FILE, (type: string) => {
			if (type === "change") {
				clear((config.watch as string));
...
function runFromConfig(config: Config) {
