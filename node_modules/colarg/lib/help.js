...
function generateHelp(optionList, commandList, usageString) {
    const maxLength = (arrString) => {
        return arrString.reduce((acc, curr) => (curr.length > acc ? curr.length : acc), 0);
    };
    ...
    const writeArray = (arr) => {
        ...
        for (const item of arr) {
            process.stdout.write(writePadded(item[0], lenName) +
